...
import { createPurchase, getDrop, getDropItem, updateDropItem } from '../../../lib/drops';
...
export const POST: APIRoute = async ({ request, locals }) => {
  try {
    ...
    if (!data.drop_id || !data.drop_item_id || !data.customer_email || !data.customer_name) {
      return new Response(JSON.stringify({ 
        error: 'Missing required fields: drop_id, drop_item_id, customer_email, customer_name' 
      }), {
        status: 400,
    ...
    const now = new Date();
    const isLive = drop.is_active && 
      (!drop.launch_date || drop.launch_date <= now) && 
      (!drop.end_date || drop.end_date > now);
    
    if (!isLive) {
      return new Response(JSON.stringify({ error: 'Drop is not currently live' }), {
        status: 400,
        headers: { 'Content-Type': 'application/json' }
      });
    }
    ...
    if (!item.is_available || item.is_sold) {
      return new Response(JSON.stringify({ error: 'Item is not available for purchase' }), {
        status: 400,
        headers: { 'Content-Type': 'application/json' }
      });
    }
    ...
    if (item.laser_file_status !== 'ready') {
      return new Response(JSON.stringify({ error: 'Item is not ready for etching yet' }), {
        status: 400,
        headers: { 'Content-Type': 'application/json' }
      });
    }
    ...
    return new Response(JSON.stringify({
      success: true,
      purchase,
      message: 'Purchase created successfully. Complete payment to confirm order.'
    }), {
      status: 201,
      headers: { 'Content-Type': 'application/json' }
    });
