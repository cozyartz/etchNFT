...
export const GET: APIRoute = async ({ request, cookies, locals }) => {
  ...
  try {
    ...
    if (!dropId) {
      return new Response(
        JSON.stringify({ error: "drop_id parameter is required" }),
        {
          status: 400,
          headers: { "Content-Type": "application/json" },
        },
...
export const POST: APIRoute = async ({ request, cookies, locals }) => {
  ...
  try {
    ...
    const {
      drop_id,
      ...itemData
    }: CreateDropItemRequest & { drop_id: string } = await request.json();
    ...
    if (
      ...
      !itemData.name ||
      !itemData.token_id ||
      !itemData.original_image_url
    ) {
      return new Response(
        JSON.stringify({
          error:
            "Missing required fields: drop_id, name, token_id, original_image_url",
        }),
        {
          status: 400,
          headers: { "Content-Type": "application/json" },
