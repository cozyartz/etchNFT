...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  try {
    ...
    if (
      ...
      console.log(`Payment confirmed for order ${orderId}, ${orders.results.length} items updated`);
    ...
    if (event?.type === 'charge:failed' && event?.data?.metadata?.order_id) {
      ...
      await db
        .prepare(`UPDATE orders SET status = 'failed', updated_at = CURRENT_TIMESTAMP WHERE id LIKE ?`)
        .bind(`${orderId}-%`)
        .run();
