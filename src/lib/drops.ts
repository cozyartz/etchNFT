...
export async function createDrop(db: D1Database, creatorId: string, data: CreateDropRequest): Promise<Drop> {
  const id = randomUUID();
  ...
  `).bind(
    id, data.name, data.description, creatorId, data.price_usd, 'USD',
    data.launch_date, data.end_date, 0, 0, data.total_supply,
    0, data.max_per_user || 1, data.product_type, data.material, data.dimensions,
    slug, JSON.stringify(data.tags || []), now, now
  ).run();
...
export async function getDropBySlug(db: D1Database, slug: string): Promise<Drop | null> {
  const result = await db.prepare(`
...
export async function getDrops(db: D1Database, filters: DropFilters = {}): Promise<Drop[]> {
...
export async function updateDrop(db: D1Database, id: string, updates: Partial<Drop>): Promise<void> {
...
export async function createDropItem(db: D1Database, dropId: string, data: CreateDropItemRequest): Promise<DropItem> {
  const id = randomUUID();
  const now = new Date().toISOString();
  ...
  `).bind(
    id, dropId, data.name, data.description, data.token_id, data.original_image_url,
    'pending', JSON.stringify(data.attributes || {}), 1, 0, now, now
  ).run();

  return item;
}

export async function getDropItems(db: D1Database, dropId: string): Promise<DropItem[]> {
  const result = await db.prepare(`
...
export async function getDropItem(db: D1Database, id: string): Promise<DropItem | null> {
  const result = await db.prepare(`
...
export async function updateDropItem(db: D1Database, id: string, updates: Partial<DropItem>): Promise<void> {
  const fields = Object.keys(updates).filter(key => key !== 'id');
  const setClause = fields.map(field => `${field} = ?`).join(', ');
...
export async function createPurchase(db: D1Database, data: PurchaseDropRequest): Promise<DropPurchase> {
  const id = randomUUID();
  const now = new Date().toISOString();
  ...
  `).bind(
    id, data.drop_id, data.drop_item_id, data.quantity, unitPrice, totalPrice,
    data.payment_method, 'pending', data.customer_email, data.customer_name,
    data.shipping_address?.line, data.shipping_address?.city, 
    data.shipping_address?.country, data.shipping_address?.postal_code,
    'pending', now, now
  ).run();
...
export async function getPurchase(db: D1Database, id: string): Promise<DropPurchase | null> {
  const result = await db.prepare(`
...
export async function getPurchases(db: D1Database, filters: { userId?: string, dropId?: string, status?: string } = {}): Promise<DropPurchase[]> {
...
export async function getDesignTemplates(db: D1Database, category?: string): Promise<DesignTemplate[]> {
...
export async function getDesignTemplate(db: D1Database, id: string): Promise<DesignTemplate | null> {
  const result = await db.prepare(`
...
export async function getDropStats(db: D1Database): Promise<DropStats> {
  const [totalDrops, activeDrops, totalSales, totalRevenue, popularProducts] = await Promise.all([
    db.prepare(`SELECT COUNT(*) as count FROM drops`).first(),
    db.prepare(`SELECT COUNT(*) as count FROM drops WHERE is_active = 1`).first(),
    db.prepare(`SELECT COUNT(*) as count FROM drop_purchases WHERE payment_status = 'completed'`).first(),
    db.prepare(`SELECT SUM(total_price_usd) as total FROM drop_purchases WHERE payment_status = 'completed'`).first(),
    db.prepare(`
  ...
  return {
    ...
    avg_price: totalSales?.count ? (totalRevenue?.total || 0) / totalSales.count : 0,
...
function formatDropItem(row: any): DropItem {
  return {
    ...
    reserved_until: row.reserved_until ? new Date(row.reserved_until) : undefined,
