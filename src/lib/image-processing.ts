...
export async function processImageForLaser(
  ...
  try {
    ...
    const qualityMetrics = calculateQualityMetrics(
      imageAnalysis.metrics!,
      enhancedImage.metrics!,
    );

    // Determine recommended laser settings
    const recommendedSettings = calculateLaserSettings(
      template,
      qualityMetrics,
    );
...
async function generateSVGFromTemplate(
  ...
  svg = svg.replace(
    "<svg",
    `<svg xmlns="http://www.w3.org/2000/svg" data-laser-ready="true" data-processing-options="${JSON.stringify(options).replace(/"/g, "&quot;")}"`,
  );
...
function calculateQualityMetrics(
  ...
  const contrast_score = Math.min(
    1.0,
    originalMetrics.contrast_score + enhancedMetrics.contrast_improvement,
  );
  const detail_score = Math.min(
    1.0,
    originalMetrics.detail_score + enhancedMetrics.edge_enhancement,
  );
...
function calculateLaserSettings(
  ...
  const baseSettings =
    materialSettings[template.material as keyof typeof materialSettings] ||
    materialSettings.wood;
